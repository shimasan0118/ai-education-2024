...
from reversi import BitBoard, Player, Window, WindowDisplay, ConsoleDisplay, MatplotlibDisplay, Game, ErrorMessage, strategies, X, Recorder
...
class Reversic:
    ...
    def __init__(self, players_info={}, sleep_time_play=2, sleep_time_turn=1, sleep_time_move=1, use_recommend=False, use_matplotlib=False):
        self.board_type = 'Square-8'
        self.player_names = {'black': 'User1', 'white': 'User2'}
        self.rec_player_names = {'black': '', 'white': ''}
        ...
        if self.use_matplotlib:
            b['User1'] = strategies.MatplotlibUserInput()
            w['User2'] = strategies.MatplotlibUserInput()
        else:
    ...
    def __start(self):
        ...
        print('\n=============================')
        print('ボードタイプ   =', self.board_type)
        print('黒石を操作するプレイヤー =', self.player_names['black'])
        print('白石を操作するプレイヤー =', self.player_names['white'])
        print('=============================\n')
    ...
    def __menu(self):
        """メニュー
        """
        print('下記のキーのいずれかを選択してください')
        print('-----------------------------')
        print(' enter  : ゲームを開始します')
        print(' t      : ボードタイプを変更します')
        print(' b      : 黒石を操作するプレイヤーを変更します')
        print(' w      : 白石を操作するプレイヤーを変更します')
        print(' q      : ゲームを終了します')
        print('-----------------------------')
        ...
        while True:
            ...
            if not user_in:
                ...
                self.player_names['black'], self.rec_player_names['black'] = self._get_player(self.players_info['black'])
                self.state = Reversic.START
                break
            elif user_in == 'w':
                self.player_names['white'], self.rec_player_names['white'] = self._get_player(self.players_info['white'])
                self.state = Reversic.START
                break
            elif user_in == 'q':
                print('See you!')
        ...
        while True:
            ...
            if re.match(r'^[1-9]+\d*$', user_in):
                ...
                if 1 <= index <= len(player_list):
                    if player_list[index-1] in ['User1', 'User2'] and self.use_recommend:
                        recommend_player = self._get_recommend_player(player_list)
                        return player_list[index-1], recommend_player
                    else:
                        return player_list[index-1], ''

    def _get_recommend_player(self, player_list):
        print('レコメンド機能を使いますか？')
        print('レコメンド機能を使うと、選択したプレイヤーのレコメンドを表示します')
        print('-----------------------------')
        print('  y :  はい')
        print('  n :  いいえ')
        while True:
            user_in = input('>> ')
            if user_in == 'y':
                print('レコメンドに使うプレイヤーを選んでください')
                print('-----------------------------')
                for num, value in enumerate(player_list, 1):
                    print(f' {num:2d} : {value}')
                print(f"  n : レコメンド機能をやめる")
                print('-----------------------------')                
                while True:
                    user_in = input('>> ')
                    if user_in == 'n':
                        return ''
                    elif re.match(r'^[1-9]+\d*$', user_in):
                        index = int(user_in)
                        if 1 <= index <= len(player_list):
                            if player_list[index-1] in ['User1', 'User2']:
                                print("ユーザー入力形式はレコメンドにできません。")
                            else:
                                return player_list[index-1]
            elif user_in == 'n':
                return ''
    ...
    def __play(self):
        ...
        # display準備
        if self.use_matplotlib:
            disp = MatplotlibDisplay(
                sleep_time_turn=self.sleep_time_turn, sleep_time_move=self.sleep_time_move, board_size=board.size
            )
        else:
            disp = ConsoleDisplay(
                sleep_time_turn=self.sleep_time_turn, sleep_time_move=self.sleep_time_move
            )
        ...
        #レコメンドプレイヤー情報
        recommend_players = {'black': {}, 'white': {}}

        for color in ('black', 'white'):
            name = self.player_names[color]
            rec_name = self.rec_player_names[color]
            selected_players[color] = Player(color, name, self.players_info[color][name]
)
            if not rec_name == '':
                recommend_players[color]['name'] = rec_name
                recommend_players[color]['strategy'] = self.players_info[color][rec_name]
        ...
        Game(
            ...
            board,
            disp,
            black_recommend_player = recommend_players['black'],
            white_recommend_player = recommend_players['white'],
        ).play()
