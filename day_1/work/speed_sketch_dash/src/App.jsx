      ...
      switch (result.status) {
        ...
        case 'result':
...

          {
            ...
            const filteredResult = result.data.filter(x => !constants.BANNED_EN_LABELS.includes(x.label));
            ...
            // console.log(applyEasyMode);
            // console.log(filteredResult[0].score);
            if (applyEasyMode > 0 && filteredResult[0].score > constants.START_REJECT_THRESHOLD) {
              ...
              for (let i = 0; i < filteredResult.length && i < amount + 1; ++i) {
                // console.log(filteredResult[i].label)
                filteredResult[i].label = constants.EN_TO_JA_LABELS[filteredResult[i].label]
                // console.log(filteredResult[i].label)
              ...
              filteredResult[0].label = constants.EN_TO_JA_LABELS[filteredResult[0].label]
            }
    ...
    <>
      ...
      {((isPlaying && gameCurrentTime !== null && targets)) && (
        ...
        <div className='absolute top-5 text-center'>
          <h2 className='text-4xl'>&quot;{targets[targetIndex]}&quot;</h2>
      ...
      {isPlaying && (
        <div className='absolute bottom-5 text-center'>
          ...
          <div className='flex gap-2 justify-center'>
            <button onClick={() => { handleClearCanvas() }}>クリア</button>
            <button onClick={() => { goNext(false) }}>スキップ</button>
            <button onClick={() => { handleEndGame(true) }}>メニューに戻る</button>
          </div>
        </div>
      )}
    </>
